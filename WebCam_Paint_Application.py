...
# Define the upper and lower boundaries for a color to be considered 
color_lower = np.array([136, 87, 111])
...
while True:
    ...
    blueMask = cv2.inRange(hsv, color_lower, color_upper)
    blueMask = cv2.erode(blueMask, kernel, iterations=2)
    blueMask = cv2.morphologyEx(blueMask, cv2.MORPH_OPEN, kernel)
    blueMask = cv2.dilate(blueMask, kernel, iterations=1)

    # Find contours in the image
    # ret , cnts , hierachy = cv2.findContours(blueMask.copy(), cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
